        ...
        rows.append(
            {
                ...
                "pearson": _pearson(sub["Post_Pre"], sub[corr_item]),
            }
        )

    m = pd.DataFrame(rows).set_index("group")
    wide = m[["n_ids", "pct", "median", "mean", "var", "pearson"]].T
    wide.index = ["該当人数", "全体に対する人数割合（％）", "点数変化の中央値", "点数変化の平均値", "点数変化の分散", "点数変化との相関係数（ピアソン）"]

    wide_display = wide.copy()
    for r in ["全体に対する人数割合（％）", "点数変化の中央値", "点数変化の平均値", "点数変化の分散", "点数変化との相関係数（ピアソン）"]:
...
def main() -> None:
    ...
    with tabs[0]:
        ...
        st.markdown("")
        st.markdown("<div style='margin-left:20px;'>・タブ名：「Versant」もしくは 「Casec」を含むもの（大文字小文字は不問）。「VERSANT①」や「１_Casec」なども可。</div>",unsafe_allow_html=True)
        st.markdown("<div style='margin-left:20px;'>・表は８列構成（列の順番は不問）：「ID」、「学習頻度」、「学習期間」、「学習日数」、「〇〇_Pre」、「〇〇_Post」、「〇〇_Post-Pre」、「Initial_Level」（大文字小文字の違い重要）。「〇〇」に「_」（アンダーバー）が含まれていてもOK</div>",unsafe_allow_html=True)
