...
def _tab2_summary(df: pd.DataFrame) -> None:
    ...
    wide = pd.DataFrame(rows).set_index("group").T
    wide.index = ["中央値(Post_Pre)", "平均(Post_Pre)", "分散(Post_Pre)", "ピアソン相関(Post_Pre, 学習日数)"]
    st.dataframe(wide, use_container_width=True)

    _download_csv_button(wide.reset_index().rename(columns={"index": "metric"}), "tab2_summary.csv", "CSVでダウンロード")

    line_df = (
        pd.DataFrame(rows)[["group", "pearson"]]
        .rename(columns={"group": item, "pearson": "ピアソン相関(Post_Pre, 学習日数)"})
        .copy()
    )
    fig = px.line(line_df, x=item, y="ピアソン相関(Post_Pre, 学習日数)", markers=True)
...
def _tab3_heatmap_scatter(df: pd.DataFrame) -> None:
    st.subheader("3項目（ヒートマップ / 散布図）")
    c1, c2, c3 = st.columns(3)
    with c1:
        x = st.selectbox("横軸の項目", options=FILTER_COLS, index=FILTER_COLS.index("学習日数"))
    with c2:
        y = st.selectbox("縦軸の項目", options=FILTER_COLS, index=FILTER_COLS.index("Post_Pre"))
    with c3:
        color = st.selectbox("色(点の大きさ)の項目", options=FILTER_COLS, index=FILTER_COLS.index("学習効率(テストの伸び/学習日数)"))
    ...
    fig_scatter = px.scatter(
        ...
        color=_to_number(scatter_df[color]) if _is_numeric_series(scatter_df[color]) else scatter_df[color].astype(str),
        size=np.abs(_to_number(scatter_df[color])) if _is_numeric_series(scatter_df[color]) else None,
    ...
    if pivot.size == 0 or pivot.dropna(how="all").dropna(axis=1, how="all").empty:
        ...
        return
    fig_hm = px.imshow(pivot, aspect="auto", labels={"x": x_label, "y": y_label, "color": f"平均({color})"})
    st.plotly_chart(fig_hm, use_container_width=True)


def _tab4_scatter(df: pd.DataFrame) -> None:
    st.subheader("2項目（散布図）")
    c1, c2 = st.columns(2)
    with c1:
        x = st.selectbox("横軸の項目", options=FILTER_COLS, index=FILTER_COLS.index("学習日数"))
    with c2:
        y = st.selectbox("縦軸の項目", options=FILTER_COLS, index=FILTER_COLS.index("学習効率(テストの伸び/学習日数)"))
...
def main() -> None:
    st.set_page_config(page_title="学習時間と伸びの分析", layout="wide")
    st.title("学習時間と伸びの分析")

    if not _check_password():
        st.stop()

    tabs = st.tabs(["①アップロード", "②KPI/相関", "③3項目可視化", "④2項目散布図"])

    with tabs[0]:
        st.write("エクセルファイルをアップロードしてください")
        st.info(
            "ファイルの形式は下記です。タブ名は「Versant」(大文字小文字関係なく) or 「Casec」(大文字小文字関係なく)の文字列を含みます。"
