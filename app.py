...
def _tab2_summary(df: pd.DataFrame) -> None:
    ...
    st.caption("※データの散らばり具合を表します。数値が大きいほどデータが散らばっています。")
...
def _tab3_heatmap_scatter(df: pd.DataFrame) -> None:
    ...
    if pivot.size == 0 or pivot.dropna(how="all").dropna(axis=1, how="all").empty:
        ...
        fig_hm.update_layout(coloraxis_colorbar=dict(tickformat=".2f"))
...
・タブ名は「Versant」(大文字小文字関係なく) or 「Casec」(大文字小文字関係なく)の文字列を含みます。
